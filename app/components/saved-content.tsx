...
const SavedContent = memo(({ isActive }: SavedContentProps) => {
  ...
  const counts = useMemo(
    () => ({
      ...
      providers: savedItems.filter(
        (i: SavedItemData) => i.itemType === "provider",
      ).length,
      products: savedItems.filter(
        (i: SavedItemData) => i.itemType === "product",
      ).length,
    }),
    [savedItems],
  );
            ...
            >
              ...
              <span
                className={`text-[9px] ml-0.5 ${
                  filter === tab.key
                    ? "text-white/60 dark:!text-slate-100"
                    : "text-slate-400"
                }`}
              >
                {tab.count}
      ...
      {isLoading ? (
        <div className="flex flex-col gap-2.5 px-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-white dark:bg-slate-800 rounded-2xl p-3 border border-slate-100 dark:border-slate-700 animate-pulse"
            >
            ...
            >
              ...
              <p className="text-[12px] text-slate-500 text-center max-w-[240px]">
                {search
                  ...
                  : "Bookmark providers and products while exploring to find them here."}
              </p>
            </motion.div>
          ) : viewMode === "list" ? (
            <div className="flex flex-col gap-2.5 px-4">
              {items.map((item: SavedItemData, i: number) => (
                ...
                >
                  ...
                  >
                    {/* Image */}
                    <div className="relative w-[88px] h-[88px] rounded-xl overflow-hidden flex-shrink-0 cursor-pointer bg-slate-100">
                      {item.image ? (
                        <img
                          src={item.image}
                          alt={item.name}
                      ...
                      {item.isOpen === false && (
                        <div className="absolute inset-0 bg-black/40 flex items-center justify-center">
                          <span className="text-white text-[9px] font-bold bg-black/50 px-2 py-0.5 rounded-full">
                            Closed
                          </span>
                        </div>
                      )}
                      <div
                        className={`absolute bottom-1.5 left-1.5 text-[7px] font-bold px-1.5 py-0.5 rounded-full ${
                          item.itemType === "provider"
                    ...
                    <div className="flex-1 min-w-0 py-0.5">
                      <div className="flex items-start justify-between gap-1.5">
                        <h3 className="text-[13px] font-bold text-slate-800 dark:text-white leading-tight line-clamp-1">
                          {item.name}
                        </h3>
                        <motion.button
                          whileTap={{ scale: 0.8 }}
              ...
              {items.map((item: SavedItemData, i: number) => (
                ...
                >
                  <div className="relative h-[120px]">
                    ...
                    {item.verified && (
                      <div className="absolute top-2 left-2 bg-emerald-500 text-white text-[7px] font-bold px-1.5 py-0.5 rounded-full">
                        ✓
                      </div>
                    )}
                    <div
                      className={`absolute bottom-2 left-2 text-[7px] font-bold px-1.5 py-0.5 rounded-full ${
                        item.itemType === "provider"
                          ? "bg-blue-500 text-white"
                          : "bg-amber-500 text-white"
                      }`}
                  ...
                  <div className="p-2.5">
                    <h3 className="text-[12px] font-bold text-slate-800 dark:text-white line-clamp-1">
                      {item.name}
                    </h3>
